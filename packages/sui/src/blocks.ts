...
const definitions: Array<{ field: string; kind: PromptBlockKind; values: readonly string[]; template?: (reference: string) => string }> = [
  ...
  { field: "design", kind: "structure", values: ["tokens.semantic", "tokens.color", "tokens.spacing", "tokens.typography", "tokens.radius", "tokens.elevation", "theme.light_dark", "density.comfortable", "motion.reduced", "feedback.status"], template: (reference) => `    design ${reference.replace(/\./g, "_")}:\n        purpose: ${reference}\n` },
  { field: "responsive", kind: "constraint", values: ["mobile_first", "compact_0_767", "medium_768_1023", "wide_1024_plus", "reflow.stack", "overflow.prevent", "touch.target_44px", "content.max_width", "sidebar.adaptive", "table.scroll_or_cards"], template: (reference) => `    responsive ${reference.replace(/\./g, "_")}:\n        strategy: ${reference}\n` },
  { field: "accessibility", kind: "constraint", values: ["keyboard.complete", "focus.visible", "focus_order.logical", "screen_reader.labels", "semantic_roles", "contrast.aa", "motion.preference", "live_region.status", "error.announced", "dialog.focus_trap"], template: (reference) => `    accessibility ${reference.replace(/\./g, "_")}:\n        require: ${reference}\n` },
  { field: "navigation", kind: "logic", values: ["route.push", "route.replace", "route.back", "modal.open_close", "tab.switch", "deep_link.supported", "unsaved_changes.confirm", "history.preserved", "skip_link.available", "focus.restore"], template: (reference) => `    navigation ${reference.replace(/\./g, "_")}:\n        behavior: ${reference}\n` },
  { field: "binding", kind: "data", values: ["input.form_value", "output.task_artifact", "state.view_model", "state.loading", "state.error", "state.empty", "selection.current", "query.search_params", "route.params", "event.user_action"], template: (reference) => `    binding ${reference.replace(/\./g, "_")}:\n        source: ${reference}\n` },
  { field: "data", kind: "data", values: ["model.view", "model.form", "model.list_item", "schema.json", "field.required", "field.optional", "format.date_time", "format.number", "pagination.cursor", "empty_state.message"], template: (reference) => `    data ${reference.replace(/\./g, "_")}:\n        model: ${reference}\n` },
